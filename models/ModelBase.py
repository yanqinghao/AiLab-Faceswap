...
class ModelBase(object):
    def __init__(
        ...
        self.is_training_mode = (
            training_data_src_path is not None and training_data_dst_path is not None
        )
        ...
        if ask_enable_autobackup and (self.iter == 0 or ask_override):
            default_autobackup = (
                False if self.iter == 0 else self.options.get("autobackup", False)
            )
            self.options["autobackup"] = io.input_bool(
                "Enable autobackup? (y/n ?:help skip:%s) : "
                % (yn_str[default_autobackup]),
                default_autobackup,
                help_message="Autobackup model files with preview every hour for last 15 hours. Latest backup located in model/<>_autobackups/01",
            )
        else:
        ...
        if ask_write_preview_history and (self.iter == 0 or ask_override):
            default_write_preview_history = (
                False
                if self.iter == 0
                else self.options.get("write_preview_history", False)
            )
        ...
        if (
            ...
            choose_preview_history = io.input_bool(
                ...
                % (yn_str[False]),
                False,
                help_message="Preview image history will stay stuck with old faces if you reuse the same model on different celebs. Choose no unless you are changing src/dst to a new person",
            )
        else:
        ...
        if ask_target_iter:
            if self.iter == 0 or ask_override:
                ...
                self.options["target_iter"] = max(
                    0,
                    io.input_int(
                        "Target iteration (skip:unlimited/default) : ", iterations
                    ),
                )
            else:
                try:
        ...
        if ask_batch_size and (self.iter == 0 or ask_override):
            default_batch_size = (
                0 if self.iter == 0 else self.options.get("batch_size", 0)
            )
            try:
            ...
            self.batch_size = max(
                ...
                io.input_int(
                    ...
                    default_batch_size,
                    help_message="Larger batch size is better for NN's generalization, but it can cause Out of Memory error. Tune this value for your videocard manually.",
                ),
            )
        else:
            try:
        ...
        if ask_sort_by_yaw:
            if self.iter == 0 or ask_override:
                ...
                self.options["sort_by_yaw"] = io.input_bool(
                    ...
                    default_sort_by_yaw,
                    help_message="NN will not learn src face directions that don't match dst face directions. Do not use if the dst face has hair that covers the jaw.",
                )
            else:
        ...
        if ask_random_flip:
            if self.iter == 0:
                self.options["random_flip"] = io.input_bool(
                    ...
                    True,
                    help_message="Predicted face will look more naturally without this option, but src faceset should cover all face directions as dst faceset.",
                )
            else:
            ...
            if self.iter == 0:
                self.options["src_scale_mod"] = np.clip(
                    io.input_int(
                        ...
                        0,
                        help_message="If src face shape is wider than dst, try to decrease this value to get a better result.",
                    ),
                    -30,
                    30,
                )
        ...
        width_value = (
            max(
                [len(str(x)) for x in self.options.values()]
                + [len(str(self.iter)), len(self.get_model_name())]
            )
            + 1
        ...
        if self.device_config.cpu_only:
            ...
            for idx in self.device_config.gpu_idxs:
                ...
                vram_str = (
                    f"{nnlib.device.getDeviceVRAMTotalGb(idx):.2f}GB"
        ...
        if (
            not self.device_config.cpu_only and self.device_config.gpu_vram_gb[0] <= 2
        ):  # Low VRAM warning
            model_summary_text += ["/!\\"]
    ...
    def get_static_preview(self):
        return self.onGetPreview(self.sample_for_preview)[0][
            1
        ]  # first preview, and bgr
                ...
                for i in range(15, 0, -1):
                    ...
                    if idx_backup_path.exists():
                        if i == 15:
                            ...
                            Path_utils.move_all_files(
                                idx_backup_path, next_idx_packup_path
                            )
                    ...
                    if i == 1:
                        ...
                        for preview, filepath in plist:
                            ...
                            img = (
                                np.concatenate([preview_lh, preview], axis=0) * 255
                            ).astype(np.uint8)
            ...
            if self.write_preview_history:
                plist += [
                    (
                        ...
                        str(self.preview_history_path / ("%.6d.jpg" % (self.iter))),
                    )
                ]

            for preview, filepath in plist:
                preview_lh = ModelBase.get_loss_history_preview(
                    self.loss_history, self.iter, preview.shape[1], preview.shape[2]
                )
    ...
    def get_strpath_storage_for_file(self, filename):
        if self.device_args["force_gpu_idx"] == -1:
            ...
            return str(
                self.model_path
                / (
                    str(self.device_args["force_gpu_idx"])
                    + "_"
                    + self.get_model_name()
                    + "_"
                    + filename
