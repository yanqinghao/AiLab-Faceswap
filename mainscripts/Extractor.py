...
class ExtractSubprocessor(Subprocessor):
    ...
    class Cli(Subprocessor.Cli):
        ...
        def on_initialize(self, client_dict):
            ...
            self.final_output_path = (
                Path(client_dict["final_output_dir"])
                if "final_output_dir" in client_dict.keys()
                else None
            )
            ...
            self.e = None
            device_config = nnlib.DeviceConfig(
                cpu_only=self.cpu_only, force_gpu_idx=self.device_idx, allow_growth=True
            )
        ...
        def process_data(self, data):
            ...
            if "rects" in self.type:
                if min(w, h) < 128:
                    self.log_err(
                        "Image is too small %s : [%d, %d]" % (str(filename_path), w, h)
                    )
                    data.rects = []
                else:
                ...
                data.landmarks = self.e.extract(
                    ...
                    data.rects,
                    self.second_pass_e
                    if (src_dflimg is None and data.landmarks_accurate)
                    else None,
                    is_bgr=True,
                )
                if data.rects_rotation != 0:
                ...
                if src_dflimg is not None and len(rects) != 1:
                    ...
                    print(
                        "src_dflimg is not None and len(rects) != 1", str(filename_path)
                    )
                    ...
                    for rect, image_landmarks in zip(rects, landmarks):
                        ...
                        if src_dflimg is not None and filename_path.suffix == ".jpg":
                            ...
                            output_file = str(
                                self.final_output_path / filename_path.name
                            )
                            if str(filename_path) != str(output_file):
                                shutil.copy(str(filename_path), str(output_file))
                        else:
                            output_file = "{}_{}{}".format(
                                str(self.final_output_path / filename_path.stem),
                                str(face_idx),
                                ".jpg",
                            )
                            cv2_imwrite(
    ...
    def __init__(
        ...
        self.devices = ExtractSubprocessor.get_devices_for_config(
            self.manual, self.type, multi_gpu, cpu_only
        )
    ...
    def get_data(self, host_dict):
        if not self.manual:
            ...
            while len(self.input_data) > 0:
                ...
                filename, data_rects, data_landmarks = (
                    data.filename,
                    data.rects,
                    data.landmarks,
                )
                ...
                if (
                    need_remark_face
                ):  # need remark image from input data that already has a marked face?
                    need_remark_face = False
                    if (
                        len(data_rects) != 0
                ...
                if len(data_rects) == 0:
                    ...
                    self.view_scale = (
                        1.0
                        if self.manual_window_size == 0
                        else self.manual_window_size / (h * (16.0 / 9.0))
                    )
                    ...
                    if self.cache_image[0] == (h, w, c) + (self.view_scale, filename):
                        ...
                        self.cache_image = (
                            (h, w, c) + (self.view_scale, filename),
                            self.image,
                        )
                    ...
                    while True:
                        ...
                        for ev in mouse_events:
                            ...
                            if ev == io.EVENT_MOUSEWHEEL and not self.rect_locked:
                                ...
                                diff = (
                                    1
                                    if new_rect_size <= 40
                                    else np.clip(new_rect_size / 10, 1, 10)
                                )
                        ...
                        if (
                            self.x != new_x
                            or self.y != new_y
                            or self.rect_size != new_rect_size
                            or self.extract_needed
                            or redraw_needed
                        ):
                            self.x = new_x
                            self.y = new_y
                            ...
                            if redraw_needed:
                                ...
                                return ExtractSubprocessor.Data(
                                    filename, landmarks_accurate=self.landmarks_accurate
                                )
                            else:
                                return ExtractSubprocessor.Data(
                                    filename,
    ...
    def on_result(self, host_dict, data, result):
        if self.manual == True:
            ...
            view_landmarks = (
                (np.array(self.landmarks) * self.view_scale).astype(np.int).tolist()
            )
    ...
    def get_devices_for_config(manual, type, multi_gpu, cpu_only):
        ...
        if "rects" in type or type == "landmarks" or type == "fanseg":
            if (
                not cpu_only and type == "rects-mt" and backend == "plaidML"
            ):  # plaidML works with MT very slowly
                cpu_only = True
            ...
            return [
                (i, "CPU", "CPU%d" % (i), 0)
                for i in (
                    range(min(8, multiprocessing.cpu_count())) if not DEBUG else [0]
                )
            ]
...
class DeletedFilesSearcherSubprocessor(Subprocessor):
    ...
    def __init__(self, input_paths, debug_paths):
        ...
        self.result = []
        super().__init__(
            "DeletedFilesSearcherSubprocessor", DeletedFilesSearcherSubprocessor.Cli, 60
        )
...
def extract_fanseg(input_dir, device_args={}):
    ...
    if paths_to_extract_len > 0:
        io.log_info(
            "Performing extract fanseg for %d files..." % (paths_to_extract_len)
        )
...
def extract_umd_csv(
    input_file_csv, image_size=256, face_type="full_face", device_args={}
):
    ...
    try:
        ...
        io.log_err(
            "Unable to open or read file " + str(input_file_csv_path) + ": " + str(e)
        )
        return
    ...
    if len(data) > 0:
        ...
        data = ExtractSubprocessor(
            data, "landmarks", multi_gpu=multi_gpu, cpu_only=cpu_only
        ).run()
    ...
    if images_found != 0:
        ...
        if manual_fix:
            if all(np.array([d.faces_detected > 0 for d in data]) == True):
                ...
                fix_data = [
                    ExtractSubprocessor.Data(d.filename)
                    for d in data
