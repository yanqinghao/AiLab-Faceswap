...
class device:
            ...
            **in_options
            ...
            if not self.cpu_only:
                ...
                if force_gpu_idxs is not None:
                    ...
                    gpu_idx = (
                        ...
                        if (
                            force_gpu_idx >= 0
                            and device.isValidDeviceIdx(force_gpu_idx)
                        )
                        else device.getBestValidDeviceIdx()
                        if not choose_worst_gpu
                        else device.getWorstValidDeviceIdx()
                    )
                    if gpu_idx != -1:
                        if self.multi_gpu:
            ...
            if not self.cpu_only:
                ...
                for gpu_idx in self.gpu_idxs:
                    ...
                    self.gpu_compute_caps += [
                        device.getDeviceComputeCapability(gpu_idx)
                    ]
    ...
    def getValidDevicesWithAtLeastTotalMemoryGB(totalmemsize_gb):
        ...
        if device.backend == "plaidML":
            for i in device.getValidDeviceIdxsEnumerator():
                if (
                    plaidML_devices[i]["globalMemSize"]
                    >= totalmemsize_gb * 1024 * 1024 * 1024
                ):
                    result.append(i)
        elif device.backend == "tensorflow":
            for i in device.getValidDeviceIdxsEnumerator():
    ...
    def getValidDevicesIdxsWithNamesList():
        if device.backend == "plaidML":
            return [
                (i, plaidML_devices[i]["description"])
                for i in device.getValidDeviceIdxsEnumerator()
            ]
        elif device.backend == "tensorflow":
            return [
                (i, nvmlDeviceGetName(nvmlDeviceGetHandleByIndex(i)).decode())
                for i in device.getValidDeviceIdxsEnumerator()
            ]
    ...
    def getDeviceIdxsEqualModel(idx):
        if device.backend == "plaidML":
            ...
            for i in device.getValidDeviceIdxsEnumerator():
                if (
                    nvmlDeviceGetName(nvmlDeviceGetHandleByIndex(i)).decode()
                    == idx_name
                ):
                    result.append(i)
    ...
    def getDeviceComputeCapability(idx):
        ...
        if device.backend == "plaidML":
            ...
            if idx < nvmlDeviceGetCount():
                result = nvmlDeviceGetCudaComputeCapability(
...
force_plaidML = (
    os.environ.get("DFL_FORCE_PLAIDML", "0") == "1"
)  # for OpenCL build , forcing using plaidML even if NVIDIA found
force_tf_cpu = (
    os.environ.get("DFL_FORCE_TF_CPU", "0") == "1"
)  # for OpenCL build , forcing using tf-cpu if plaidML failed
has_nvml = False
has_nvml_cap = False
...
def get_plaidML_devices():
    ...
    if plaidML_devices is None:
        ...
        try:
            ...
            ] = "false"  # this enables work plaidML without run 'plaidml-setup'
            import plaidml
...
if device.backend is None and not force_tf_cpu:
    ...
    try:
        ...
        device.backend = (
            "tensorflow"
        )  # set tensorflow backend in order to use device.*device() functions

        gpu_idxs = device.getAllDevicesIdxsList()
        gpu_caps = np.array(
            [device.getDeviceComputeCapability(gpu_idx) for gpu_idx in gpu_idxs]
        )

        if len(np.ndarray.flatten(np.argwhere(gpu_caps >= tf_min_req_cap))) == 0:
            if not force_plaidML:
                print(
                    "No CUDA devices found with minimum required compute capability: %d.%d. Falling back to OpenCL mode."
                    % (tf_min_req_cap // 10, tf_min_req_cap % 10)
                )
            device.backend = None
            nvmlShutdown()
        else:
