...
def main(args, device_args):
    ...
    thread = threading.Thread(
        target=trainerThread, args=(s2c, c2s, e, args, device_args)
    )
    thread.start()
    ...
    if no_preview:
        ...
        while True:
            ...
            if update_preview:
                ...
                for i in range(0, len(head_lines)):
                    ...
                    head[t:b, 0:w] += imagelib.get_text_image(
                        (head_line_height, w, c), head_lines[i], color=[0.8] * c
                    )
            ...
            key, chr_key, ctrl_pressed, alt_pressed, shift_pressed = (
                key_events[-1] if len(key_events) > 0 else (0, 0, False, False, False)
            )
