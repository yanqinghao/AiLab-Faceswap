    ...
    def configure_backend(self):
        ...
        while True:
            ...
            if selection not in ("1", "2", "3"):
                print(
                    "'{}' is not a valid selection. Please try again".format(selection)
                )
                continue
            break
...
def cv2_read_img(filename, raise_error=False):
    ...
    try:
        ...
        success = False
        msg = (
            "Error while reading image. This is most likely caused by special characters in "
...
def hash_encode_image(image, extension):
    ...
    ).hexdigest()  # pylint:disable=no-member,c-extension-no-member
    return f_hash, img
    ...
    for attempt in range(attempts):
        try:
            ...
            out = subprocess.check_output(
                cmd, stderr=subprocess.STDOUT, shell=iswin, timeout=timeout
            )
...
def deprecation_warning(func_name, additional_info=None):
    ...
    msg = "{}  has been deprecated and will be removed from a future update.".format(
        func_name
    )
...
def rotate_landmarks(face, rotation_matrix):
    ...
    logger = logging.getLogger(__name__)  # pylint:disable=invalid-name
    logger.trace(
        "Rotating landmarks: (rotation_matrix: %s, type(face): %s",
        rotation_matrix,
        type(face),
    )
    ...
    # Alignments Dict
        ...
        bounding_box = [
            ...
            [face.get("x", 0) + face.get("w", 0), face.get("y", 0)],
            [face.get("x", 0) + face.get("w", 0), face.get("y", 0) + face.get("h", 0)],
            [face.get("x", 0), face.get("y", 0) + face.get("h", 0)],
        ]
    ...
    for item in (bounding_box, landmarks):
        ...
        transformed = cv2.transform(
            points, rotation_matrix  # pylint:disable=no-member
        ).astype(np.int32)
...
class GetModel:
    ...
    def __init__(self, model_filename, cache_dir, git_model_id):
        ...
        self.url_base = os.path.join(
            os.path.dirname(os.path.abspath(sys.argv[0])), "zipmodel"
        )
    ...
    def _url_download(self):
        ...
        retval = os.path.join(
            self.url_base, tag, "{}.zip".format(self._model_full_name)
        )
