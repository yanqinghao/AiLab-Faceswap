...
class InteractBase(object):
    ...
    def named_window(self, wnd_name):
        if wnd_name not in self.named_windows:
            ...
            print("named_window: ", wnd_name, " already created.")
    ...
    def destroy_window(self, wnd_name):
        if wnd_name in self.named_windows:
            ...
            if wnd_name == self.focus_wnd_name:
                self.focus_wnd_name = (
                    list(self.named_windows.keys())[-1]
                    if len(self.named_windows) != 0
                    else None
                )
    ...
    def capture_mouse(self, wnd_name):
        if wnd_name in self.named_windows:
            ...
            print("capture_mouse: named_window ", wnd_name, " not found.")
    ...
    def capture_keys(self, wnd_name):
        if wnd_name in self.named_windows:
            if wnd_name not in self.capture_keys_windows:
                ...
                print("capture_keys: already set for window ", wnd_name)
        else:
            print("capture_keys: named_window ", wnd_name, " not found.")

    def progress_bar(self, desc, total, leave=True, initial=0):
        if self.pg_bar is None:
            self.pg_bar = tqdm(
                total=total, desc=desc, leave=leave, ascii=True, initial=initial
            )
        else:
            print("progress_bar: already set.")
    ...
    def add_key_event(
        self, wnd_name, ord_key, ctrl_pressed, alt_pressed, shift_pressed
    ):
        if wnd_name not in self.key_events:
            self.key_events[wnd_name] = []
        self.key_events[wnd_name] += [
            (ord_key, chr(ord_key), ctrl_pressed, alt_pressed, shift_pressed)
        ]
    ...
    def input_bool(self, s, default_value, help_message=None):
        while True:
            try:
                ...
                return bool(
                    {"y": True, "n": False, "1": True, "0": False}.get(
                        inp.lower(), default_value
                    )
                )
            except:
    ...
    def input_in_time(self, str, max_time_sec):
        ...
        p = multiprocessing.Process(
            target=self.input_process, args=(sys.stdin.fileno(), sq, str)
        )
        p.start()
        t = time.time()
        inp = False
...
class InteractDesktop(InteractBase):
    ...
    def on_capture_mouse(self, wnd_name):
        ...
        def onMouse(event, x, y, flags, param):
            ...
            if event == cv2.EVENT_LBUTTONDOWN:
                ev = InteractBase.EVENT_LBUTTONDOWN
            elif event == cv2.EVENT_LBUTTONUP:
                ev = InteractBase.EVENT_LBUTTONUP
            elif event == cv2.EVENT_RBUTTONDOWN:
                ev = InteractBase.EVENT_RBUTTONDOWN
            elif event == cv2.EVENT_RBUTTONUP:
                ev = InteractBase.EVENT_RBUTTONUP
            elif event == cv2.EVENT_MBUTTONDOWN:
                ev = InteractBase.EVENT_MBUTTONDOWN
            elif event == cv2.EVENT_MBUTTONUP:
                ev = InteractBase.EVENT_MBUTTONUP
            elif event == cv2.EVENT_MOUSEWHEEL:
                ev = InteractBase.EVENT_MOUSEWHEEL
                x, y = (
                    self.last_xy
                )  # fix opencv bug when window size more than screen size
            else:
                ev = 0
    ...
    def on_process_messages(self, sleep_time=0):
        ...
        if has_capture_keys and ord_key != -1:
            self.add_key_event(
                self.focus_wnd_name, ord_key, False, False, shift_pressed
            )
...
class InteractDocker(InteractColab):
